...
export abstract class MusicSheetCalculator {
  ...
  public prepareGraphicalMusicSheet(): void {
    ...
    const measureList: GraphicalMeasure[][] = this.graphicalMusicSheet
      .MeasureList;
    ...
    for (
      let idx: number = 0, len: number = musicSheet.SourceMeasures.length;
      idx < len;
      ++idx
    ) {
  ...
  public calculate(): void {
    ...
    this.calculateXLayout(
      this.graphicalMusicSheet,
      this.maxInstrNameLabelLength()
    );
    ...
    GraphicalMusicSheet.transformRelativeToAbsolutePosition(
      this.graphicalMusicSheet
    );
  }

  public calculateXLayout(
    ...
    if (this.graphicalMusicSheet.MeasureList.length > 0) {
      ...
      let measures: GraphicalMeasure[] = this.graphicalMusicSheet
        .MeasureList[0];
      let minimumStaffEntriesWidth: number = this.calculateMeasureXLayout(
        measures
      );
      minimumStaffEntriesWidth = this.calculateMeasureWidthFromLyrics(
        measures,
      ...
      minLength =
        minimumStaffEntriesWidth * 1.2 +
        maxInstrNameLabelLength +
        maxInstructionsLength;
      for (
        let i: number = 1;
        i < this.graphicalMusicSheet.MeasureList.length;
        i++
      ) {
        ...
        minLength = Math.max(
          minLength,
          minimumStaffEntriesWidth * 1.2 + maxInstructionsLength
        );
      }
    }
  ...
  public calculateMeasureWidthFromLyrics(
  ...
  protected calculateMusicSystemsRelativePositions(
    ...
    let relativePosition: PointF2D = new PointF2D(
      this.rules.PageLeftMargin + this.rules.SystemLeftMargin,
      0
    );
    ...
    if (graphicalMusicPage === graphicalMusicPage.Parent.MusicPages[0]) {
      if (EngravingRules.Rules.RenderTitle) {
        relativePosition.y +=
          this.rules.TitleTopDistance +
          this.rules.SheetTitleHeight +
          this.rules.TitleBottomDistance;
      }
    } else {
      if (EngravingRules.Rules.RenderTitle) {
        relativePosition.y +=
          this.rules.PageTopMargin + this.rules.TitleTopDistance;
      } else {
    ...
    for (let i: number = 1; i < graphicalMusicPage.MusicSystems.length; i++) {
      ...
      relativePosition = new PointF2D(
        this.rules.PageLeftMargin + this.rules.SystemLeftMargin,
        0
      );

      // find optimum distance between Systems
      const previousSystem: MusicSystem =
        graphicalMusicPage.MusicSystems[i - 1];
      const lastPreviousStaffLine: StaffLine =
        previousSystem.StaffLines[previousSystem.StaffLines.length - 1];
      const distance: number =
        lastPreviousStaffLine.SkyBottomLineCalculator.getBottomLineMax() -
        this.rules.StaffHeight +
        Math.abs(
          musicSystem.StaffLines[0].SkyBottomLineCalculator.getSkyLineMin()
        ) +
        this.rules.MinimumAllowedDistanceBetweenSystems;

      relativePosition.y =
        previousSystem.PositionAndShape.RelativePosition.y +
        lastPreviousStaffLine.PositionAndShape.RelativePosition.y +
        this.rules.StaffHeight +
        Math.max(this.rules.SystemDistance, distance);
  ...
  protected optimizeDistanceBetweenStaffLines(musicSystem: MusicSystem): void {
    ...
    if (!musicSystem.checkStaffEntriesForStaffEntryLink()) {
      for (let i: number = 0; i < musicSystem.StaffLines.length - 1; i++) {
        const upperBottomLine: number = musicSystem.StaffLines[
          i
        ].SkyBottomLineCalculator.getBottomLineMax();
  ...
  protected calculateMeasureNumberPlacement(musicSystem: MusicSystem): void {
    ...
    for (const measure of staffLine.Measures) {
      ...
      if (
        (measure.MeasureNumber === currentMeasureNumber ||
          measure.MeasureNumber ===
            currentMeasureNumber + this.rules.MeasureNumberLabelOffset) &&
        ...
        if (
          ...
          this.calculateSingleMeasureNumberPlacement(
            measure,
            staffLine,
            musicSystem
          );
  ...
  private calculateSingleMeasureNumberPlacement(
    ...
    const graphicalLabel: GraphicalLabel = new GraphicalLabel(
      new Label(labelNumber),
      this.rules.MeasureNumberLabelHeight,
      TextAlignmentEnum.LeftBottom
    );

    const skyBottomLineCalculator: SkyBottomLineCalculator =
      staffLine.SkyBottomLineCalculator;
    ...
    let end: number =
      relativeX -
      graphicalLabel.PositionAndShape.BorderLeft +
    ...
    if (
      staffLine === musicSystem.StaffLines[0] &&
      musicSystem === musicSystem.Parent.MusicSystems[0]
    ) {
    ...
    const skyLineMinValue: number = skyBottomLineCalculator.getSkyLineMinInRange(
      start,
      end
    );
    ...
    if (measure === staffLine.Measures[0]) {
      ...
      if (musicSystem.GroupBrackets.length > 0) {
        for (const groupBracket of musicSystem.GroupBrackets) {
          minBracketTopBorder = Math.min(
            minBracketTopBorder,
            groupBracket.PositionAndShape.BorderTop
          );
        }
      }
    ...
    graphicalLabel.PositionAndShape.RelativePosition = new PointF2D(
      relativeX,
      relativeY
    );
    ...
    skyBottomLineCalculator.updateSkyLineInRange(
      ...
      relativeY + graphicalLabel.PositionAndShape.BorderMarginTop
    );
  ...
  protected calculateSingleStaffLineLyricsPosition(
    ...
    for (let idx: number = 0; idx < len; ++idx) {
      ...
      const measureRelativePosition: PointF2D =
        measure.PositionAndShape.RelativePosition;
      ...
      for (let idx2: number = 0; idx2 < len2; ++idx2) {
        ...
        if (staffEntry.LyricsEntries.length > 0) {
          ...
          for (let i: number = 0; i < staffEntry.LyricsEntries.length; i++) {
            const lyricsEntryLabel: GraphicalLabel =
              staffEntry.LyricsEntries[i].GraphicalLabel;
            minMarginLeft = Math.min(
              minMarginLeft,
              staffEntryPositionX +
                lyricsEntryLabel.PositionAndShape.BorderMarginLeft
            );
            maxMarginRight = Math.max(
              maxMarginRight,
              staffEntryPositionX +
                lyricsEntryLabel.PositionAndShape.BorderMarginRight
            );
          }

          // check BottomLine in this range and take the maximum between the two values
          const bottomLineMax: number = skyBottomLineCalculator.getBottomLineMaxInRange(
            minMarginLeft,
            maxMarginRight
          );
    ...
    for (const staffEntry of lyricsStaffEntriesList) {
      ...
      for (let i: number = 0; i < staffEntry.LyricsEntries.length; i++) {
        ...
        const sortedLyricVerseNumberIndex: number = lyricVersesNumber.indexOf(
          verseNumber
        );
        const firstPosition: number =
          lyricsStartYPosition +
        ...
        let position: number =
          firstPosition +
          (this.rules.VerticalBetweenLyricsDistance + this.rules.LyricsHeight) *
            sortedLyricVerseNumberIndex;
        if (this.leadSheet) {
          position =
            3.4 +
            (this.rules.VerticalBetweenLyricsDistance +
        ...
        const previousRelativeX: number =
          lyricsEntryLabel.PositionAndShape.RelativePosition.x;
        lyricsEntryLabel.PositionAndShape.RelativePosition = new PointF2D(
          previousRelativeX,
          position
        );
    ...
    if (lyricsStaffEntriesList.length > 0) {
      ...
      let startX: number =
        lyricsStaffEntriesList[0].PositionAndShape.RelativePosition.x +
        lyricsStaffEntriesList[0].PositionAndShape.BorderMarginLeft +
        lyricsStaffEntriesList[0].parentMeasure.PositionAndShape
          .RelativePosition.x;
      startX = startX > endX ? endX : startX;
      skyBottomLineCalculator.updateBottomLineInRange(
        startX,
        endX,
        maxPosition
      );
  ...
  protected calculateLyricsExtendsAndDashes(
    lyricsStaffEntries: GraphicalStaffEntry[]
  ): void {
    // iterate again to create now the extend lines and dashes for words
    for (
      let idx: number = 0, len: number = lyricsStaffEntries.length;
      idx < len;
      ++idx
    ) {
      ...
      for (let i: number = 0; i < staffEntry.LyricsEntries.length; i++) {
        ...
        if (
          lyricEntry.ParentLyricWord !== undefined &&
          lyricEntry.ParentLyricWord.GraphicalLyricsEntries[
            lyricEntry.ParentLyricWord.GraphicalLyricsEntries.length - 1
          ] !== lyricEntry
        ) {
  ...
  protected clearRecreatedObjects(): void {
    ...
    for (
      let idx: number = 0,
        len: number = this.staffEntriesWithGraphicalTies.length;
      idx < len;
      ++idx
    ) {
      const staffEntriesWithGraphicalTie: GraphicalStaffEntry = this
        .staffEntriesWithGraphicalTies[idx];
  ...
  protected handleStaffEntryLink(
    graphicalStaffEntry: GraphicalStaffEntry,
    staffEntryLinks: StaffEntryLink[]
  ): void {
  ...
  protected calculateMusicSystems(): void {
    ...
    const allMeasures: GraphicalMeasure[][] = this.graphicalMusicSheet
      .MeasureList;
    ...
    for (
      let idx: number = 0, len: number = allMeasures.length;
      idx < len && idx < EngravingRules.Rules.MaxMeasureToDrawIndex;
      ++idx
    ) {
      const graphicalMeasures: GraphicalMeasure[] = allMeasures[idx];
      const visiblegraphicalMeasures: GraphicalMeasure[] = [];
      for (
        let idx2: number = 0, len2: number = graphicalMeasures.length;
        idx2 < len2;
        ++idx2
      ) {
    ...
    for (
      let idx: number = 0, len: number = visibleMeasureList.length;
      idx < len;
      ++idx
    ) {
    ...
    for (
      let idx: number = 0,
        len: number = this.graphicalMusicSheet.MusicPages.length;
      idx < len;
      ++idx
    ) {
      const graphicalMusicPage: GraphicalMusicPage = this.graphicalMusicSheet
        .MusicPages[idx];
      for (
        let idx2: number = 0,
          len2: number = graphicalMusicPage.MusicSystems.length;
        idx2 < len2;
        ++idx2
      ) {
    ...
    for (
      ...
      for (
        ...
        const musicSystem: MusicSystem = graphicalMusicPage.MusicSystems[idx2];
        for (
          let idx3: number = 0, len3: number = musicSystem.StaffLines.length;
          idx3 < len3;
          ++idx3
        ) {
    ...
    for (
      ...
      for (
        ...
        if (!this.leadSheet) {
          musicSystem.setYPositionsToVerticalLineObjectsAndCreateLines(
            this.rules
          );
          musicSystem.createSystemLeftLine(
            this.rules.SystemThinLineWidth,
            this.rules.SystemLabelsRightMargin
          );
          musicSystem.createInstrumentBrackets(
            this.graphicalMusicSheet.ParentMusicSheet.Instruments,
            this.rules.StaffHeight
          );
          musicSystem.createGroupBrackets(
            this.graphicalMusicSheet.ParentMusicSheet.InstrumentalGroups,
            this.rules.StaffHeight,
            0
          );
      ...
      const distance: number =
        graphicalMusicPage.MusicSystems[0].PositionAndShape.BorderTop;
      for (
        let idx2: number = 0,
          len2: number = graphicalMusicPage.MusicSystems.length;
        idx2 < len2;
        ...
        musicSystem.PositionAndShape.RelativePosition = new PointF2D(
          musicSystem.PositionAndShape.RelativePosition.x,
          musicSystem.PositionAndShape.RelativePosition.y - distance
        );
      }
      for (
    ...
    for (const musicPage of this.graphicalMusicSheet.MusicPages) {
      for (const musicSystem of musicPage.MusicSystems) {
        for (const staffLine of musicSystem.StaffLines) {
          const sbc: SkyBottomLineCalculator =
            staffLine.SkyBottomLineCalculator;
          for (const measure of staffLine.Measures) {
            for (const staffEntry of measure.staffEntries) {
              ...
              const gps: BoundingBox =
                staffEntry.graphicalChordContainer.PositionAndShape;
              const start: number =
                gps.BorderMarginLeft + sps.AbsolutePosition.x;
              const end: number =
                gps.BorderMarginRight + sps.AbsolutePosition.x;
  ...
  protected calculateGraphicalVerbalContinuousDynamic(
    graphicalContinuousDynamic: GraphicalContinuousDynamicExpression,
    ...
    const left: number =
      startPosInStaffline.x + graphLabel.PositionAndShape.BorderMarginLeft;
    const right: number =
      startPosInStaffline.x + graphLabel.PositionAndShape.BorderMarginRight;
    // placement always below the currentStaffLine, with the exception of Voice Instrument (-> above)
    const placement: PlacementEnum =
      graphicalContinuousDynamic.ContinuousDynamic.Placement;
    ...
    if (placement === PlacementEnum.Below) {
      drawingHeight = skyBottomLineCalculator.getBottomLineMaxInRange(
        left,
        right
      ); // Bottom line
      graphLabel.PositionAndShape.RelativePosition = new PointF2D(
        startPosInStaffline.x,
        drawingHeight - graphLabel.PositionAndShape.BorderMarginTop
      );
    } else {
      drawingHeight = skyBottomLineCalculator.getSkyLineMinInRange(left, right);
      graphLabel.PositionAndShape.RelativePosition = new PointF2D(
        startPosInStaffline.x,
        drawingHeight - graphLabel.PositionAndShape.BorderMarginBottom
      );
    }
  }
   ...
   * @param startPosInStaffline Starting point in staff line
   */
  public calculateGraphicalContinuousDynamic(
    graphicalContinuousDynamic: GraphicalContinuousDynamicExpression,
    startPosInStaffline: PointF2D
  ): void {
    const staffIndex: number =
      graphicalContinuousDynamic.ParentStaffLine.ParentStaff.idInMusicSheet;
    ...
    const endMeasure: GraphicalMeasure = this.graphicalMusicSheet.getGraphicalMeasureFromSourceMeasureAndIndex(
      graphicalContinuousDynamic.ContinuousDynamic.EndMultiExpression
        .SourceMeasureParent,
      staffIndex
    );
    if (!endMeasure) {
    ...
    const endAbsoluteTimestamp: Fraction = Fraction.createFromFraction(
      graphicalContinuousDynamic.ContinuousDynamic.EndMultiExpression
        .AbsoluteTimestamp
    );

    const endPosInStaffLine: PointF2D = this.getRelativePositionInStaffLineFromTimestamp(
      endAbsoluteTimestamp,
      staffIndex,
      endStaffLine,
      endStaffLine.isPartOfMultiStaffInstrument(),
      0
    );
    ...
    const currentStaffLineIndex: number = musicSystem.StaffLines.indexOf(
      staffLine
    );
    const skyBottomLineCalculator: SkyBottomLineCalculator =
      staffLine.SkyBottomLineCalculator;
    ...
    const sameStaffLine: boolean =
      endStaffLine !== undefined && staffLine === endStaffLine;
    ...
    const lowerStartX: number =
      endStaffLine.Measures[0].beginInstructionsWidth -
    ...
    if (placement === PlacementEnum.Below) {
      ...
      if (nextStaffLineIndex > currentStaffLineIndex) {
        ...
        const nextStaffLine: StaffLine =
          musicSystem.StaffLines[nextStaffLineIndex];

        const distanceBetweenStaffLines: number =
          nextStaffLine.PositionAndShape.RelativePosition.y -
      ...
      let maxBottomLineValueForExpressionLength: number = skyBottomLineCalculator.getBottomLineMaxInRange(
        upperStartX,
        upperEndX
      );
      ...
      if (currentStaffLineIndex < musicSystem.StaffLines.length - 1) {
        ...
        const closestToEndStaffEntry: GraphicalStaffEntry = staffLine.findClosestStaffEntry(
          upperEndX
        );
        const closestToStartStaffEntry: GraphicalStaffEntry = staffLine.findClosestStaffEntry(
          upperStartX
        );

        if (closestToStartStaffEntry && closestToEndStaffEntry) {
          // must check both StaffLines
          const startVerticalContainer: VerticalGraphicalStaffEntryContainer =
            closestToStartStaffEntry.parentVerticalContainer;
          ...
          if (withinCrossedBeam) {
            const nextStaffLine: StaffLine =
              musicSystem.StaffLines[nextStaffLineIndex];
            const nextStaffLineMinSkyLineValue: number = nextStaffLine.SkyBottomLineCalculator.getSkyLineMinInRange(
              upperStartX,
              upperEndX
            );
            const distanceBetweenStaffLines: number =
              nextStaffLine.PositionAndShape.RelativePosition.y -
              staffLine.PositionAndShape.RelativePosition.y;
            const relativeSkyLineHeight: number =
              distanceBetweenStaffLines + nextStaffLineMinSkyLineValue;

            if (
              relativeSkyLineHeight - this.rules.WedgeOpeningLength >
              this.rules.StaffHeight
            ) {
      ...
      if (!sameStaffLine) {
        maxBottomLineValueForExpressionLength = endStaffLine.SkyBottomLineCalculator.getBottomLineMaxInRange(
          lowerStartX,
          lowerEndX
        );
      ...
      if (staffLine.ParentStaff.ParentInstrument.Staves.length === 1) {
        ...
        const previousStaffLine: StaffLine =
          musicSystem.StaffLines[previousStaffLineIndex];
        const distanceBetweenStaffLines: number =
          staffLine.PositionAndShape.RelativePosition.y -
      ...
      let minSkyLineValueForExpressionLength: number = skyBottomLineCalculator.getSkyLineMinInRange(
        upperStartX,
        upperEndX
      );
      ...
      // special case - wedge must be drawn within the boundaries of a crossedBeam
      if (
        staffLine.ParentStaff.ParentInstrument.Staves.length > 1 &&
        currentStaffLineIndex > 0
      ) {
        // find GraphicalStaffEntries closest to wedge's xPositions
        const closestToStartStaffEntry: GraphicalStaffEntry = staffLine.findClosestStaffEntry(
          upperStartX
        ...
        if (closestToStartStaffEntry && closestToEndStaffEntry) {
          ...
          if (withinCrossedBeam) {
            const formerStaffLine: StaffLine =
              musicSystem.StaffLines[formerStaffLineIndex];
            const formerStaffLineMaxBottomLineValue: number = formerStaffLine.SkyBottomLineCalculator.getBottomLineMaxInRange(
              upperStartX,
              upperEndX
            );
            const distanceBetweenStaffLines: number =
            ...
            const relativeSkyLineHeight: number =
              distanceBetweenStaffLines - formerStaffLineMaxBottomLineValue;
            idealY =
              (relativeSkyLineHeight - this.rules.StaffHeight) / 2 +
              this.rules.StaffHeight;
          }
        }
      }
      ...
      // do the same in case of a Wedge ending at another StaffLine
      if (!sameStaffLine) {
        minSkyLineValueForExpressionLength = endStaffLine.SkyBottomLineCalculator.getSkyLineMinInRange(
          lowerStartX,
          lowerEndX
        );
    ...
    if (
      graphicalContinuousDynamic.ContinuousDynamic.DynamicType ===
      ...
      if (sameStaffLine) {
        ...
        graphicalContinuousDynamic.createFirstHalfCrescendoLines(
          upperStartX,
          upperEndX,
          idealY
        );
        ...
        secondGraphicalContinuousDynamic.createSecondHalfCrescendoLines(
          lowerStartX,
          lowerEndX,
          secondIdealY
        );
      ...
      if (sameStaffLine) {
        graphicalContinuousDynamic.createDiminuendoLines(
          upperStartX,
          upperEndX,
          idealY
        );
        ...
        graphicalContinuousDynamic.createFirstHalfDiminuendoLines(
          upperStartX,
          upperEndX,
          idealY
        );
        ...
        secondGraphicalContinuousDynamic.createSecondHalfDiminuendoLines(
          lowerStartX,
          lowerEndX,
          secondIdealY
        );
  ...
  protected calculateGraphicalInstantaneousDynamicExpression(
    graphicalInstantaneousDynamic: GraphicalInstantaneousDynamicExpression,
    startPosInStaffline: PointF2D
  ): void {
    ...
    const left: number =
      startPosInStaffline.x +
      graphicalInstantaneousDynamic.PositionAndShape.BorderMarginLeft;
    const right: number =
      startPosInStaffline.x +
      graphicalInstantaneousDynamic.PositionAndShape.BorderMarginRight;
    const skyBottomLineCalculator: SkyBottomLineCalculator =
      staffLine.SkyBottomLineCalculator;
    ...
    if (graphicalInstantaneousDynamic.Placement === PlacementEnum.Above) {
      const skyLineValue: number = skyBottomLineCalculator.getSkyLineMinInRange(
        left,
        right
      );
      ...
      if (
        ...
        staffLine.ParentStaff !==
          staffLine.ParentStaff.ParentInstrument.Staves[0]
      ) {
        const formerStaffLine: StaffLine =
          staffLine.ParentMusicSystem.StaffLines[
            staffLine.ParentMusicSystem.StaffLines.indexOf(staffLine) - 1
          ];
        ...
        if (skyLineValue > -difference / 2) {
          ...
          yPosition =
            skyLineValue -
            graphicalInstantaneousDynamic.PositionAndShape.BorderMarginBottom;
        }
      } else {
        yPosition =
          skyLineValue -
      ...
      graphicalInstantaneousDynamic.PositionAndShape.RelativePosition = new PointF2D(
        startPosInStaffline.x,
        yPosition
      );
    } else if (
      graphicalInstantaneousDynamic.Placement === PlacementEnum.Below
    ) {
      const bottomLineValue: number = skyBottomLineCalculator.getBottomLineMaxInRange(
        left,
        right
      );
      // if StaffLine part of multiStaff Instrument and not the last one, ideal yPosition middle of distance between Staves
      const lastStaff: Staff =
        staffLine.ParentStaff.ParentInstrument.Staves[
          staffLine.ParentStaff.ParentInstrument.Staves.length - 1
        ];
      if (
        staffLine.isPartOfMultiStaffInstrument() &&
        staffLine.ParentStaff !== lastStaff
      ) {
        const nextStaffLine: StaffLine =
          staffLine.ParentMusicSystem.StaffLines[
            staffLine.ParentMusicSystem.StaffLines.indexOf(staffLine) + 1
        ...
        // take always into account the size of the Dynamic
        if (
          bottomLineValue + border <
          this.rules.StaffHeight + difference / 2
        ) {
          yPosition = this.rules.StaffHeight + difference / 2;
        } else {
          yPosition =
            bottomLineValue -
            graphicalInstantaneousDynamic.PositionAndShape.BorderMarginTop;
        }
      } else {
        yPosition =
          bottomLineValue -
  ...
  protected layoutSingleRepetitionEnding(
    start: GraphicalMeasure,
    end: GraphicalMeasure,
    numberText: string,
    offset: number,
    leftOpen: boolean,
    rightOpen: boolean
  ...
  protected calculateLabel(
    ...
    let left: number =
      relative.x + graphLabel.PositionAndShape.BorderMarginLeft;
    let right: number =
      relative.x + graphLabel.PositionAndShape.BorderMarginRight;

    // check if GraphicalLabel exceeds the StaffLine's borders.
    if (right > staffLine.PositionAndShape.Size.width) {
      right =
        staffLine.PositionAndShape.Size.width - this.rules.MeasureRightMargin;
    ...
    if (placement === PlacementEnum.Below) {
      skyBottomLineCalculator.updateBottomLineInRange(
        left,
        right,
        graphLabel.PositionAndShape.BorderMarginBottom + drawingHeight
      );
    } else {
      skyBottomLineCalculator.updateSkyLineInRange(
        left,
        right,
        graphLabel.PositionAndShape.BorderMarginTop + drawingHeight
      );
    }
    return graphLabel;
  }
  ...
  protected calculateTempoExpressionsForMultiTempoExpression(
    sourceMeasure: SourceMeasure,
    multiTempoExpression: MultiTempoExpression,
    measureIndex: number
  ): void {
    // calculate absolute Timestamp
    const absoluteTimestamp: Fraction = Fraction.plus(
      sourceMeasure.AbsoluteTimestamp,
      multiTempoExpression.Timestamp
    );
    const measures: GraphicalMeasure[] = this.graphicalMusicSheet.MeasureList[
      measureIndex
    ];
    let relative: PointF2D = new PointF2D();

    if (
      multiTempoExpression.ContinuousTempo ||
      multiTempoExpression.InstantaneousTempo
    ) {
      // TempoExpressions always on the first visible System's StaffLine
      let staffLine: StaffLine = measures[0].ParentStaffLine;
      let firstVisibleMeasureX: number =
        measures[0].PositionAndShape.RelativePosition.x;
      let verticalIndex: number = 0;
      for (let j: number = 0; j < measures.length; j++) {
        if (
          !measures[j].ParentStaffLine ||
          measures[j].ParentStaffLine.Measures.length === 0
        ) {
          continue;
        }
        ...
        if (measures[j].ParentStaffLine.Measures.length > 0) {
          ...
          firstVisibleMeasureX =
            measures[j].PositionAndShape.RelativePosition.x;
          verticalIndex = j;
          break;
        }
      }
      relative = this.getRelativePositionInStaffLineFromTimestamp(
        absoluteTimestamp,
        verticalIndex,
        staffLine,
        staffLine.isPartOfMultiStaffInstrument(),
      ...
      if (
        ...
        multiTempoExpression.EntriesList[0].Expression instanceof
          InstantaneousTempoExpression
      ) {
        const instantaniousTempo: InstantaneousTempoExpression = multiTempoExpression
          .EntriesList[0].Expression as InstantaneousTempoExpression;
        ...
        if (
          staffLine.Measures[0].staffEntries.length > 0 &&
          Math.abs(
            relative.x -
              staffLine.Measures[0].staffEntries[0].PositionAndShape
                .RelativePosition.x
          ) === 0 &&
          staffLine.ParentMusicSystem ===
            staffLine.ParentMusicSystem.Parent.MusicSystems[0]
        ) {
          const firstInstructionEntry: GraphicalStaffEntry =
            staffLine.Measures[0].FirstInstructionStaffEntry;
      ...
      for (const entry of multiTempoExpression.EntriesList) {
        ...
        if (entry.Expression instanceof InstantaneousTempoExpression) {
          ...
          for (const expr of staffLine.AbstractExpressions) {
            if (
              expr instanceof GraphicalInstantaneousTempoExpression &&
              (expr.SourceExpression as AbstractTempoExpression).Label ===
          ...
          const graphicalTempoExpr: GraphicalInstantaneousTempoExpression = new GraphicalInstantaneousTempoExpression(
            entry.Expression,
            graphLabel
          );
          ...
          if (
            (entry.Expression as InstantaneousTempoExpression).Enum ===
            TempoEnum.metronomeMark
          ) {
            this.createMetronomeMark(
              entry.Expression as InstantaneousTempoExpression
            );
            continue;
          }
    ...
    for (
      ...
      for (
        ...
        for (
          ...
          const staffLine: StaffLine = musicSystem.StaffLines[idx3];
          for (
            let idx4: number = 0, len4: number = staffLine.Measures.length;
            idx4 < len4;
            ++idx4
          ) {
            ...
            if (graphicalMeasure.FirstInstructionStaffEntry !== undefined) {
              ...
              if (index > -1) {
                graphicalMeasure.PositionAndShape.ChildElements.splice(
                  index,
                  1
                );
              }
  ...
  protected handleVoiceEntry(
    voiceEntry: VoiceEntry,
    graphicalStaffEntry: GraphicalStaffEntry,
    accidentalCalculator: AccidentalCalculator,
    openLyricWords: LyricWord[],
    activeClef: ClefInstruction,
    openTuplets: Tuplet[],
    openBeams: Beam[],
    ...
    sourceStaffEntry: SourceStaffEntry = undefined
  ): OctaveEnum {
    if (
      voiceEntry.StemDirectionXml !== StemDirectionType.Undefined &&
    ...
    const gve: GraphicalVoiceEntry = graphicalStaffEntry.findOrCreateGraphicalVoiceEntry(
      voiceEntry
    );
    gve.octaveShiftValue = octaveShiftValue;
    for (
      let idx: number = 0, len: number = voiceEntry.Notes.length;
      idx < len;
      ++idx
    ) {
      ...
      if (
        sourceStaffEntry !== undefined &&
        sourceStaffEntry.Link !== undefined &&
        linkedNotes !== undefined &&
        linkedNotes.indexOf(note) > -1
      ) {
        continue;
      }
      let graphicalNote: GraphicalNote;
      if (voiceEntry.IsGrace) {
        graphicalNote = MusicSheetCalculator.symbolFactory.createGraceNote(
          note,
          gve,
          activeClef,
          octaveShiftValue
        ...
        graphicalNote = MusicSheetCalculator.symbolFactory.createNote(
          note,
          gve,
          activeClef,
          octaveShiftValue,
      ...
      graphicalStaffEntry.addGraphicalNoteToListAtCorrectYPosition(
        gve,
        graphicalNote
      );
    ...
    if (voiceEntry.Articulations.length > 0) {
      this.handleVoiceEntryArticulations(
        voiceEntry.Articulations,
        voiceEntry,
        graphicalStaffEntry
      );
    }
    if (voiceEntry.TechnicalInstructions.length > 0) {
      this.handleVoiceEntryTechnicalInstructions(
        voiceEntry.TechnicalInstructions,
        voiceEntry,
        graphicalStaffEntry
      );
    }
    if (voiceEntry.LyricsEntries.size() > 0) {
      this.handleVoiceEntryLyrics(
        voiceEntry,
        graphicalStaffEntry,
        openLyricWords
      );
    ...
    if (voiceEntry.OrnamentContainer !== undefined) {
      this.handleVoiceEntryOrnaments(
        voiceEntry.OrnamentContainer,
        voiceEntry,
        graphicalStaffEntry
      );
    }
  ...
  protected layoutVoiceEntries(graphicalStaffEntry: GraphicalStaffEntry): void {
    graphicalStaffEntry.PositionAndShape.RelativePosition = new PointF2D(
      0.0,
      0.0
    );
    if (!this.leadSheet) {
      for (const gve of graphicalStaffEntry.graphicalVoiceEntries) {
        ...
        const voiceEntry: VoiceEntry =
          graphicalNotes[0].sourceNote.ParentVoiceEntry;
        const hasPitchedNote: boolean =
          graphicalNotes[0].sourceNote.Pitch !== undefined;
        this.layoutVoiceEntry(
          voiceEntry,
          graphicalNotes,
          graphicalStaffEntry,
          hasPitchedNote
  ...
  protected maxInstrNameLabelLength(): number {
    ...
    for (const instrument of this.graphicalMusicSheet.ParentMusicSheet
      .Instruments) {
  ...
  protected calculateSheetLabelBoundingBoxes(): void {
    ...
    if (musicSheet.Title !== undefined && EngravingRules.Rules.RenderTitle) {
      const title: GraphicalLabel = new GraphicalLabel(
        musicSheet.Title,
        this.rules.SheetTitleHeight,
    ...
    if (
      ...
      EngravingRules.Rules.RenderSubtitle
    ) {
      const subtitle: GraphicalLabel = new GraphicalLabel(
        musicSheet.Subtitle,
        this.rules.SheetSubtitleHeight,
    ...
    if (
      musicSheet.Composer !== undefined &&
      EngravingRules.Rules.RenderComposer
    ) {
      const composer: GraphicalLabel = new GraphicalLabel(
        musicSheet.Composer,
        this.rules.SheetComposerHeight,
        TextAlignmentEnum.RightCenter
      );
    ...
    if (
      musicSheet.Lyricist !== undefined &&
      EngravingRules.Rules.RenderLyricist
    ) {
      const lyricist: GraphicalLabel = new GraphicalLabel(
        musicSheet.Lyricist,
        this.rules.SheetAuthorHeight,
        TextAlignmentEnum.LeftCenter
      );
  ...
  protected checkMeasuresForWholeRestNotes(): void {
    for (
      ...
      const musicPage: GraphicalMusicPage = this.graphicalMusicSheet.MusicPages[
        idx
      ];
      for (
        let idx2: number = 0, len2: number = musicPage.MusicSystems.length;
        idx2 < len2;
        ++idx2
      ) {
        ...
        for (
          ...
          for (
            ...
            if (measure.staffEntries.length === 1) {
              ...
              if (
                gse.graphicalVoiceEntries.length > 0 &&
                gse.graphicalVoiceEntries[0].notes.length === 1
              ) {
                const graphicalNote: GraphicalNote =
                  gse.graphicalVoiceEntries[0].notes[0];
                if (
                  graphicalNote.sourceNote.Pitch === undefined &&
                  new Fraction(1, 2).lt(graphicalNote.sourceNote.Length)
                ) {
  ...
  protected optimizeRestNotePlacement(
    ...
    const voice1Notes: GraphicalNote[] =
      graphicalStaffEntry.graphicalVoiceEntries[0].notes;
    if (voice1Notes.length === 0) {
      return;
    }
    const voice1Note1: GraphicalNote = voice1Notes[0];
    const voice1Note1IsRest: boolean =
      voice1Note1.sourceNote.Pitch === undefined;
    if (graphicalStaffEntry.graphicalVoiceEntries.length === 2) {
      let voice2Note1IsRest: boolean = false;
      const voice2Notes: GraphicalNote[] =
        graphicalStaffEntry.graphicalVoiceEntries[1].notes;
      ...
      if (voice1Note1IsRest && voice2Note1IsRest) {
        this.calculateTwoRestNotesPlacementWithCollisionDetection(
          graphicalStaffEntry
        );
      ...
      voice1Note1IsRest &&
      graphicalStaffEntry !== measure.staffEntries[0] &&
      graphicalStaffEntry !==
        measure.staffEntries[measure.staffEntries.length - 1]
    ) {
      const staffEntryIndex: number = measure.staffEntries.indexOf(
        graphicalStaffEntry
      );
      const previousStaffEntry: GraphicalStaffEntry =
        measure.staffEntries[staffEntryIndex - 1];
      const nextStaffEntry: GraphicalStaffEntry =
        measure.staffEntries[staffEntryIndex + 1];
      if (previousStaffEntry.graphicalVoiceEntries.length === 1) {
        const previousNote: GraphicalNote =
          previousStaffEntry.graphicalVoiceEntries[0].notes[0];
        if (
          previousNote.sourceNote.NoteBeam !== undefined &&
          nextStaffEntry.graphicalVoiceEntries.length === 1
        ) {
          const nextNote: GraphicalNote =
            nextStaffEntry.graphicalVoiceEntries[0].notes[0];
          if (
            nextNote.sourceNote.NoteBeam !== undefined &&
  ...
  protected getRelativePositionInStaffLineFromTimestamp(
    ...
    const numEntries: number = this.graphicalMusicSheet
      .VerticalGraphicalStaffEntryContainers.length;
    const index: number = this.graphicalMusicSheet.GetInterpolatedIndexInVerticalContainers(
      timestamp
    );
    ...
    leftStaffEntry = this.getFirstLeftNotNullStaffEntryFromContainer(
      leftIndex,
      verticalIndex,
    ...
    if (leftStaffEntry !== undefined && rightStaffEntry !== undefined) {
      let measureRelativeX: number =
        leftStaffEntry.parentMeasure.PositionAndShape.RelativePosition.x;
      if (firstVisibleMeasureRelativeX > 0) {
        measureRelativeX = firstVisibleMeasureRelativeX;
      }
      let leftX: number =
        leftStaffEntry.PositionAndShape.RelativePosition.x + measureRelativeX;
      let rightX: number =
        rightStaffEntry.PositionAndShape.RelativePosition.x +
        rightStaffEntry.parentMeasure.PositionAndShape.RelativePosition.x;
      if (firstVisibleMeasureRelativeX > 0) {
        rightX =
          rightStaffEntry.PositionAndShape.RelativePosition.x +
          measureRelativeX;
      ...
      if (leftStaffEntry !== rightStaffEntry) {
        ...
        const leftDifference: Fraction = Fraction.minus(
          timestamp,
          leftTimestamp
        );
        timestampQuotient =
          leftDifference.RealValue /
          Fraction.minus(rightTimestamp, leftTimestamp).RealValue;
      }
      if (
        leftStaffEntry.parentMeasure.ParentStaffLine !==
        rightStaffEntry.parentMeasure.ParentStaffLine
      ) {
      ...
      relative = new PointF2D(
        leftX + (rightX - leftX) * timestampQuotient,
        0.0
      );
    }
    return relative;
  ...
  protected getRelativeXPositionFromTimestamp(timestamp: Fraction): number {
    ...
    const discreteIndex: number = Math.max(
      0,
      Math.min(Math.round(index), numEntries - 1)
    );
    const gse: GraphicalStaffEntry = this.graphicalMusicSheet.VerticalGraphicalStaffEntryContainers[
      discreteIndex
    ].getFirstNonNullStaffEntry();
    const posX: number =
      gse.PositionAndShape.RelativePosition.x +
      gse.parentMeasure.PositionAndShape.RelativePosition.x;
    return posX;
  }
  ...
  protected calculatePageLabels(page: GraphicalMusicPage): void {
    ...
    if (page.MusicSystems.length > 0) {
      ...
      firstSystemAbsoluteTopMargin =
        firstMusicSystem.PositionAndShape.RelativePosition.y +
        firstMusicSystem.PositionAndShape.BorderTop;
    }
    ...
    if (this.graphicalMusicSheet.Composer !== undefined) {
      ...
      const relative: PointF2D = new PointF2D();
      relative.x =
        this.graphicalMusicSheet.ParentMusicSheet.pageWidth -
        this.rules.PageRightMargin;
      relative.y =
  ...
  protected createGraphicalTies(): void {
    for (
      ...
      measureIndex <
      this.graphicalMusicSheet.ParentMusicSheet.SourceMeasures.length;
      measureIndex++
    ) {
      const sourceMeasure: SourceMeasure = this.graphicalMusicSheet
        .ParentMusicSheet.SourceMeasures[measureIndex];
      for (
        let staffIndex: number = 0;
        staffIndex < sourceMeasure.CompleteNumberOfStaves;
        staffIndex++
      ) {
        for (
          let j: number = 0;
          j < sourceMeasure.VerticalSourceStaffEntryContainers.length;
          j++
        ) {
          const sourceStaffEntry: SourceStaffEntry =
            sourceMeasure.VerticalSourceStaffEntryContainers[j].StaffEntries[
              staffIndex
            ];
          if (sourceStaffEntry !== undefined) {
            ...
            for (
              let idx: number = 0,
                len: number = sourceStaffEntry.VoiceEntries.length;
              idx < len;
              ++idx
            ) {
              const voiceEntry: VoiceEntry = sourceStaffEntry.VoiceEntries[idx];
              for (
                let idx2: number = 0, len2: number = voiceEntry.Notes.length;
                idx2 < len2;
                ++idx2
              ) {
  ...
  private handleTie(
    tie: Tie,
    startGraphicalStaffEntry: GraphicalStaffEntry,
    staffIndex: number,
    measureIndex: number
  ): void {
    let startGse: GraphicalStaffEntry = startGraphicalStaffEntry;
    let startNote: GraphicalNote = startGse.findEndTieGraphicalNoteFromNote(
      tie.StartNote
    );
    ...
    for (let i: number = 1; i < tie.Notes.length; i++) {
      ...
      endGse = this.graphicalMusicSheet.GetGraphicalFromSourceStaffEntry(
        tie.Notes[i].ParentStaffEntry
      );
      if (!endGse) {
        continue;
      }
      ...
      if (
        ...
        if (
          !startNote.sourceNote.PrintObject ||
          !endNote.sourceNote.PrintObject
        ) {
          continue;
        }
        const graphicalTie: GraphicalTie = this.createGraphicalTie(
          tie,
          startGse,
          endGse,
          startNote,
  ...
  private createAccidentalCalculators(): AccidentalCalculator[] {
    ...
    if (firstSourceMeasure !== undefined) {
      for (
        let i: number = 0;
        i < firstSourceMeasure.CompleteNumberOfStaves;
        i++
      ) {
        ...
        if (firstSourceMeasure.FirstInstructionsStaffEntries[i] !== undefined) {
          for (
            let idx: number = 0,
              len: number =
                firstSourceMeasure.FirstInstructionsStaffEntries[i].Instructions
                  .length;
            idx < len;
            ++idx
          ) {
            const abstractNotationInstruction: AbstractNotationInstruction =
              firstSourceMeasure.FirstInstructionsStaffEntries[i].Instructions[
                idx
              ];
            if (abstractNotationInstruction instanceof KeyInstruction) {
              const keyInstruction: KeyInstruction = <KeyInstruction>(
                abstractNotationInstruction
              );
  ...
  private calculateVerticalContainersList(): void {
    const numberOfEntries: number = this.graphicalMusicSheet.MeasureList[0]
      .length;
    for (
      let i: number = 0;
      i < this.graphicalMusicSheet.MeasureList.length;
      ...
      for (let j: number = 0; j < numberOfEntries; j++) {
        const measure: GraphicalMeasure = this.graphicalMusicSheet.MeasureList[
          i
        ][j];
        for (
          let idx: number = 0, len: number = measure.staffEntries.length;
          idx < len;
          ++idx
        ) {
          const graphicalStaffEntry: GraphicalStaffEntry =
            measure.staffEntries[idx];
          const verticalContainer: VerticalGraphicalStaffEntryContainer = this.graphicalMusicSheet.getOrCreateVerticalContainer(
            graphicalStaffEntry.getAbsoluteTimestamp()
          );
  ...
  private setIndicesToVerticalGraphicalContainers(): void {
    for (
      let i: number = 0;
      i < this.graphicalMusicSheet.VerticalGraphicalStaffEntryContainers.length;
      i++
    ) {
      this.graphicalMusicSheet.VerticalGraphicalStaffEntryContainers[
        i
      ].Index = i;
    }
  }
  ...
  private createGraphicalMeasuresForSourceMeasure(
    sourceMeasure: SourceMeasure,
    accidentalCalculators: AccidentalCalculator[],
    openLyricWords: LyricWord[],
    openOctaveShifts: OctaveShiftParams[],
    ...
    for (
      ...
      const measure: GraphicalMeasure = this.createGraphicalMeasure(
        ...
        openTuplets,
        openBeams,
        accidentalCalculators[staffIndex],
        activeClefs,
        openOctaveShifts,
        openLyricWords,
        staffIndex,
  ...
  private createGraphicalMeasure(
    ...
    const staff: Staff = this.graphicalMusicSheet.ParentMusicSheet.getStaffFromIndex(
      staffIndex
    );
    const measure: GraphicalMeasure = MusicSheetCalculator.symbolFactory.createGraphicalMeasure(
      sourceMeasure,
      staff
    );
    ...
    if (sourceMeasure.FirstInstructionsStaffEntries[staffIndex] !== undefined) {
      for (
        let idx: number = 0,
          len: number =
            sourceMeasure.FirstInstructionsStaffEntries[staffIndex].Instructions
              .length;
        idx < len;
        ++idx
      ) {
        const instruction: AbstractNotationInstruction =
          sourceMeasure.FirstInstructionsStaffEntries[staffIndex].Instructions[
            idx
          ];
        if (instruction instanceof KeyInstruction) {
          ...
          if (
            this.graphicalMusicSheet.ParentMusicSheet.Transpose !== 0 &&
            measure.ParentStaff.ParentInstrument.MidiInstrumentId !==
    ...
    for (
      let idx: number = 0,
        len: number = sourceMeasure.StaffLinkedExpressions[staffIndex].length;
      idx < len;
      ++idx
    ) {
      const multiExpression: MultiExpression =
        sourceMeasure.StaffLinkedExpressions[staffIndex][idx];
      if (multiExpression.OctaveShiftStart !== undefined) {
        ...
        openOctaveShifts[staffIndex] = new OctaveShiftParams(
          openOctaveShift,
          multiExpression.AbsoluteTimestamp,
    ...
    for (
      ...
      entryIndex < sourceMeasure.VerticalSourceStaffEntryContainers.length;
      entryIndex++
    ) {
      const sourceStaffEntry: SourceStaffEntry =
        sourceMeasure.VerticalSourceStaffEntryContainers[entryIndex]
          .StaffEntries[staffIndex];
      if (sourceStaffEntry !== undefined) {
        for (
          let idx: number = 0,
            len: number = sourceStaffEntry.Instructions.length;
          idx < len;
          ++idx
        ) {
          const abstractNotationInstruction: AbstractNotationInstruction =
            sourceStaffEntry.Instructions[idx];
          if (abstractNotationInstruction instanceof ClefInstruction) {
            activeClefs[staffIndex] = <ClefInstruction>(
              abstractNotationInstruction
            );
          }
        }
        const graphicalStaffEntry: GraphicalStaffEntry = MusicSheetCalculator.symbolFactory.createStaffEntry(
          sourceStaffEntry,
          measure
        );
        ...
        if (openOctaveShifts[staffIndex] !== undefined) {
          const octaveShiftParams: OctaveShiftParams =
            openOctaveShifts[staffIndex];
          if (
            octaveShiftParams.getAbsoluteStartTimestamp.lte(
              sourceStaffEntry.AbsoluteTimestamp
            ) &&
            sourceStaffEntry.AbsoluteTimestamp.lte(
              octaveShiftParams.getAbsoluteEndTimestamp
            )
          ) {
        ...
        if (sourceStaffEntry.Instructions.length > 0) {
          const clefInstruction: ClefInstruction = <ClefInstruction>(
            sourceStaffEntry.Instructions[0]
          );
    ...
    if (sourceMeasure.LastInstructionsStaffEntries[staffIndex] !== undefined) {
      const lastStaffEntry: SourceStaffEntry =
        sourceMeasure.LastInstructionsStaffEntries[staffIndex];
      for (
        let idx: number = 0, len: number = lastStaffEntry.Instructions.length;
        idx < len;
        ++idx
      ) {
        const abstractNotationInstruction: AbstractNotationInstruction =
          lastStaffEntry.Instructions[idx];
        if (abstractNotationInstruction instanceof ClefInstruction) {
          activeClefs[staffIndex] = <ClefInstruction>(
            abstractNotationInstruction
          );
    ...
    for (
      ...
      const multiExpression: MultiExpression =
        sourceMeasure.StaffLinkedExpressions[staffIndex][idx];
      if (
        multiExpression.OctaveShiftEnd !== undefined &&
        openOctaveShifts[staffIndex] !== undefined &&
        multiExpression.OctaveShiftEnd ===
          openOctaveShifts[staffIndex].getOpenOctaveShift
      ) {
        openOctaveShifts[staffIndex] = undefined;
      }
    }
    ...
    if (measure.staffEntries.length === 0) {
      const sourceStaffEntry: SourceStaffEntry = new SourceStaffEntry(
        new VerticalSourceStaffEntryContainer(
      ...
      const voiceEntry: VoiceEntry = new VoiceEntry(
        new Fraction(0, 1),
        staff.Voices[0],
        sourceStaffEntry
      );
      const note: Note = new Note(
        voiceEntry,
        sourceStaffEntry,
        Fraction.createFromFraction(sourceMeasure.Duration),
        undefined
      );
      ...
      const gve: GraphicalVoiceEntry = MusicSheetCalculator.symbolFactory.createVoiceEntry(
        voiceEntry,
        graphicalStaffEntry
      );
  ...
  private checkNoteForAccidental(
    ...
    accidentalCalculator: AccidentalCalculator,
    activeClef: ClefInstruction,
    octaveEnum: OctaveEnum
  ): void {
    let pitch: Pitch = graphicalNote.sourceNote.Pitch;
    const transpose: number = this.graphicalMusicSheet.ParentMusicSheet
      .Transpose;
    if (
      transpose !== 0 &&
      graphicalNote.sourceNote.ParentStaffEntry.ParentStaff.ParentInstrument
        .MidiInstrumentId !== MidiInstrument.Percussion
    ) {
  ...
  private handleStaffEntries(): void {
    for (
      let idx: number = 0,
        len: number = this.graphicalMusicSheet.MeasureList.length;
      idx < len;
      ++idx
    ) {
      const measures: GraphicalMeasure[] = this.graphicalMusicSheet.MeasureList[
      ...
      for (
        let idx2: number = 0, len2: number = measures.length;
        idx2 < len2;
        ++idx2
      ) {
        ...
        for (const graphicalStaffEntry of measure.staffEntries) {
          if (
            graphicalStaffEntry.parentMeasure !== undefined &&
      ...
      for (
        ...
        for (
          ...
          for (
            ...
            const measure: GraphicalMeasure = staffLine.Measures[idx4];
            for (
              let idx5: number = 0, len5: number = measure.staffEntries.length;
              idx5 < len5;
              ++idx5
            ) {
              const staffEntry: GraphicalStaffEntry =
                measure.staffEntries[idx5];
  ...
  private calculateStaffEntryArticulationMarks(): void {
    for (
      ...
      for (
        let idx2: number = 0, len2: number = page.MusicSystems.length;
        idx2 < len2;
        ++idx2
      ) {
        const system: MusicSystem = page.MusicSystems[idx2];
        for (
          let idx3: number = 0, len3: number = system.StaffLines.length;
          idx3 < len3;
          ++idx3
        ) {
          const line: StaffLine = system.StaffLines[idx3];
          for (
            let idx4: number = 0, len4: number = line.Measures.length;
            idx4 < len4;
            ++idx4
          ) {
            ...
            for (
              ...
              for (
                let idx6: number = 0,
                  len6: number =
                    graphicalStaffEntry.sourceStaffEntry.VoiceEntries.length;
                idx6 < len6;
                ++idx6
              ) {
                const voiceEntry: VoiceEntry =
                  graphicalStaffEntry.sourceStaffEntry.VoiceEntries[idx6];
                if (voiceEntry.Articulations.length > 0) {
                  this.layoutArticulationMarks(
                    voiceEntry.Articulations,
                    voiceEntry,
                    graphicalStaffEntry
                  );
            ...
            for (
              ...
              for (
                ...
                if (voiceEntry.OrnamentContainer !== undefined) {
                  if (
                    voiceEntry.hasTie() &&
                    !graphicalStaffEntry.relInMeasureTimestamp.Equals(
                  ...
                  if (
                    !(
                      this.staffEntriesWithOrnaments.indexOf(
                        graphicalStaffEntry
                      ) !== -1
                    )
                  ) {
  ...
  private calculateTwoRestNotesPlacementWithCollisionDetection(
    ...
    const firstRestNote: GraphicalNote =
      graphicalStaffEntry.graphicalVoiceEntries[0].notes[0];
    const secondRestNote: GraphicalNote =
      graphicalStaffEntry.graphicalVoiceEntries[1].notes[0];
  ...
  private calculateRestNotePlacementWithCollisionDetectionFromGraphicalNote(
    ...
    if (
      graphicalStaffEntry.graphicalVoiceEntries[0].notes[0].sourceNote.Pitch ===
      undefined
    ) {
    ...
    graphicalStaffEntry.PositionAndShape.calculateAbsolutePositionsRecursiveWithoutTopelement();
    for (
      let idx: number = 0, len: number = graphicalNotes.length;
      idx < len;
      ++idx
    ) {
      const graphicalNote: GraphicalNote = graphicalNotes[idx];
      if (
        restNote.PositionAndShape.marginCollisionDetection(
          graphicalNote.PositionAndShape
        )
      ) {
        collision = true;
        break;
    ...
    if (collision) {
      if (
        restNote.sourceNote.ParentVoiceEntry.ParentVoice instanceof LinkedVoice
      ) {
        const bottomBorder: number =
          graphicalNotes[0].PositionAndShape.BorderMarginBottom +
          graphicalNotes[0].PositionAndShape.RelativePosition.y;
        restNote.PositionAndShape.RelativePosition = new PointF2D(
          0.0,
          bottomBorder - restNote.PositionAndShape.BorderMarginTop + 0.5
        );
      } else {
        const last: GraphicalNote = graphicalNotes[graphicalNotes.length - 1];
        const topBorder: number =
          last.PositionAndShape.BorderMarginTop +
          last.PositionAndShape.RelativePosition.y;
        if (
          graphicalNotes[0].sourceNote.ParentVoiceEntry.ParentVoice instanceof
          LinkedVoice
        ) {
          restNote.PositionAndShape.RelativePosition = new PointF2D(
            0.0,
            topBorder - restNote.PositionAndShape.BorderMarginBottom - 0.5
          );
        } else {
          const bottomBorder: number =
            graphicalNotes[0].PositionAndShape.BorderMarginBottom +
          ...
          if (bottomBorder < 2.0) {
            ...
            restNote.PositionAndShape.RelativePosition = new PointF2D(
              0.0,
              topBorder - restNote.PositionAndShape.BorderMarginBottom - 0.0
            );
          }
        }
      }
    ...
    for (
      ...
      for (
        ...
        for (
          ...
          const staffLine: StaffLine = musicSystem.StaffLines[idx3];
          for (
            let idx4: number = 0, len5: number = staffLine.Measures.length;
            idx4 < len5;
            ++idx4
          ) {
            const measure: GraphicalMeasure = staffLine.Measures[idx4];
            for (
              let idx6: number = 0, len6: number = measure.staffEntries.length;
              idx6 < len6;
              ++idx6
            ) {
              const staffEntry: GraphicalStaffEntry =
                measure.staffEntries[idx6];
              const graphicalTies: GraphicalTie[] = staffEntry.GraphicalTies;
              for (
                let idx7: number = 0, len7: number = graphicalTies.length;
                idx7 < len7;
                ++idx7
              ) {
                const graphicalTie: GraphicalTie = graphicalTies[idx7];
                if (
                  graphicalTie.StartNote !== undefined &&
                  graphicalTie.StartNote.parentVoiceEntry.parentStaffEntry ===
                    staffEntry
                ) {
                  const tieIsAtSystemBreak: boolean =
                    graphicalTie.StartNote.parentVoiceEntry.parentStaffEntry
                      .parentMeasure.ParentStaffLine !==
                    graphicalTie.EndNote.parentVoiceEntry.parentStaffEntry
                      .parentMeasure.ParentStaffLine;
  ...
  private calculateLyricsPosition(): void {
    const lyricStaffEntriesDict: Dictionary<
      StaffLine,
      GraphicalStaffEntry[]
    > = new Dictionary<StaffLine, GraphicalStaffEntry[]>();
    // sort the lyriceVerseNumbers for every Instrument that has Lyrics
    for (
      let idx: number = 0,
        len: number = this.graphicalMusicSheet.ParentMusicSheet.Instruments
          .length;
      idx < len;
      ++idx
    ) {
      const instrument: Instrument = this.graphicalMusicSheet.ParentMusicSheet
        .Instruments[idx];
    ...
    for (
      ...
      for (
        ...
        for (
          ...
          const staffLine: StaffLine = musicSystem.StaffLines[idx3];
          const lyricsStaffEntries: GraphicalStaffEntry[] = this.calculateSingleStaffLineLyricsPosition(
            staffLine,
            staffLine.ParentStaff.ParentInstrument.LyricVersesNumbers
          );
          lyricStaffEntriesDict.setValue(staffLine, lyricsStaffEntries);
          this.calculateLyricsExtendsAndDashes(
            lyricStaffEntriesDict.getValue(staffLine)
          );
        }
      }
    }
  ...
  private calculateSingleLyricWord(lyricEntry: GraphicalLyricEntry): void {
    ...
    const index: number = graphicalLyricWord.GraphicalLyricsEntries.indexOf(
      lyricEntry
    );
    ...
    const startStaffLine: StaffLine = <StaffLine>(
      lyricEntry.StaffEntryParent.parentMeasure.ParentStaffLine
    );
    const nextStaffLine: StaffLine = <StaffLine>(
      nextLyricEntry.StaffEntryParent.parentMeasure.ParentStaffLine
    );
    ...
    if (
      ...
      const y: number =
        lyricEntry.GraphicalLabel.PositionAndShape.RelativePosition.y;
      ...
      if (endX - startX > this.rules.MinimumDistanceBetweenDashes * 3) {
        ...
        numberOfDashes =
          Math.floor(
            (endX - startX) / this.rules.MinimumDistanceBetweenDashes
          ) - 1;
      }
      ...
      const startX: number =
        ...
        startStaffEntry.PositionAndShape.RelativePosition.x +
        lyricEntry.GraphicalLabel.PositionAndShape.BorderMarginRight;
      const lastGraphicalMeasure: GraphicalMeasure =
        startStaffLine.Measures[startStaffLine.Measures.length - 1];
      const endX: number =
        lastGraphicalMeasure.PositionAndShape.RelativePosition.x +
        lastGraphicalMeasure.PositionAndShape.Size.width;
      let y: number =
        lyricEntry.GraphicalLabel.PositionAndShape.RelativePosition.y;
      ...
      if (
        ...
        !(
          ...
          endStaffentry.parentMeasure ===
            endStaffentry.parentMeasure.ParentStaffLine.Measures[0]
        )
      ) {
        const secondStartX: number =
          nextStaffLine.Measures[0].staffEntries[0].PositionAndShape
            .RelativePosition.x;
        const secondEndX: number =
          endStaffentry.parentMeasure.PositionAndShape.RelativePosition.x +
          endStaffentry.PositionAndShape.RelativePosition.x +
          nextLyricEntry.GraphicalLabel.PositionAndShape.BorderMarginLeft;
  ...
  private calculateDashes(
    ...
    if (distance < this.rules.MinimumDistanceBetweenDashes * 3) {
      ...
      const numberOfDashes: number =
        Math.floor(distance / this.rules.MinimumDistanceBetweenDashes) - 1;
  ...
  private calculateLyricExtend(lyricEntry: GraphicalLyricEntry): void {
    let startY: number =
      lyricEntry.GraphicalLabel.PositionAndShape.RelativePosition.y;
    const startStaffEntry: GraphicalStaffEntry = lyricEntry.StaffEntryParent;
    const startStaffLine: StaffLine =
    ...
    const staffIndex: number =
      startStaffEntry.parentMeasure.ParentStaff.idInMusicSheet;
    for (
      let index: number = startStaffEntry.parentVerticalContainer.Index + 1;
      index <
      this.graphicalMusicSheet.VerticalGraphicalStaffEntryContainers.length;
      ++index
    ) {
      const gse: GraphicalStaffEntry = this.graphicalMusicSheet
        .VerticalGraphicalStaffEntryContainers[index].StaffEntries[staffIndex];
      if (gse === undefined) {
        continue;
      }
      if (gse.hasOnlyRests()) {
    ...
    if (startStaffLine === endStaffLine) {
      ...
      // start and end on different StaffLines
      // start margin from the text Label until the End of StaffLine
      const lastMeasureBb: BoundingBox =
        startStaffLine.Measures[startStaffLine.Measures.length - 1]
          .PositionAndShape;
      const startX: number =
        startStaffEntry.parentMeasure.PositionAndShape.RelativePosition.x +
        ...
        lyricEntry.GraphicalLabel.PositionAndShape.BorderMarginRight;
      const endX: number =
        lastMeasureBb.RelativePosition.x + lastMeasureBb.Size.width;
      ...
      if (
        ...
        const secondStartX: number =
          endStaffLine.Measures[0].staffEntries[0].PositionAndShape
            .RelativePosition.x;
        const secondEndX: number =
          endStaffEntry.parentMeasure.PositionAndShape.RelativePosition.x +
          endStaffEntry.PositionAndShape.RelativePosition.x +
  ...
  private calculateSingleLyricWordWithUnderscore(
    ...
    const graphicalLine: GraphicalLine = new GraphicalLine(
      lineStart,
      lineEnd,
      this.rules.LyricUnderscoreLineWidth
    );
  ...
  private calculateRightAndLeftDashesForLyricWord(
    ...
    return (
      rightDash.PositionAndShape.RelativePosition.x -
      leftDash.PositionAndShape.RelativePosition.x
    );
  }
  ...
  private calculateDynamicExpressions(): void {
    const maxIndex: number = Math.min(
      this.graphicalMusicSheet.ParentMusicSheet.SourceMeasures.length,
      EngravingRules.Rules.MaxMeasureToDrawIndex
    );
    for (let i: number = 0; i < maxIndex; i++) {
      const sourceMeasure: SourceMeasure = this.graphicalMusicSheet
        .ParentMusicSheet.SourceMeasures[i];
      for (
        let j: number = 0;
        j < sourceMeasure.StaffLinkedExpressions.length;
        j++
      ) {
        if (
          this.graphicalMusicSheet.MeasureList[i][j].ParentStaff
            .ParentInstrument.Visible
        ) {
          for (
            let k: number = 0;
            k < sourceMeasure.StaffLinkedExpressions[j].length;
            k++
          ) {
            if (
              sourceMeasure.StaffLinkedExpressions[j][k]
                .InstantaneousDynamic !== undefined ||
              (sourceMeasure.StaffLinkedExpressions[j][k]
                .StartingContinuousDynamic !== undefined &&
                sourceMeasure.StaffLinkedExpressions[j][k]
                  .StartingContinuousDynamic.StartMultiExpression ===
                  sourceMeasure.StaffLinkedExpressions[j][k] &&
                sourceMeasure.StaffLinkedExpressions[j][k].UnknownList
                  .length === 0)
            ) {
              this.calculateDynamicExpressionsForMultiExpression(
                sourceMeasure.StaffLinkedExpressions[j][k],
                i,
                j
              );
  ...
  private calculateOctaveShifts(): void {
    for (
      let i: number = 0;
      i < this.graphicalMusicSheet.ParentMusicSheet.SourceMeasures.length;
      i++
    ) {
      const sourceMeasure: SourceMeasure = this.graphicalMusicSheet
        .ParentMusicSheet.SourceMeasures[i];
      for (
        ...
        if (
          ...
          for (
            ...
            if (
              sourceMeasure.StaffLinkedExpressions[j][k].OctaveShiftStart !==
              undefined
            ) {
              this.calculateSingleOctaveShift(
                sourceMeasure,
                sourceMeasure.StaffLinkedExpressions[j][k],
                i,
                j
  ...
  private getFirstLeftNotNullStaffEntryFromContainer(
    ...
    if (
      this.graphicalMusicSheet.VerticalGraphicalStaffEntryContainers[
        horizontalIndex
      ].StaffEntries[verticalIndex] !== undefined
    ) {
      return this.graphicalMusicSheet.VerticalGraphicalStaffEntryContainers[
        horizontalIndex
      ].StaffEntries[verticalIndex];
    }
    for (let i: number = horizontalIndex - 1; i >= 0; i--) {
      if (
        this.graphicalMusicSheet.VerticalGraphicalStaffEntryContainers[i]
          .StaffEntries[verticalIndex] !== undefined
      ) {
        return this.graphicalMusicSheet.VerticalGraphicalStaffEntryContainers[i]
          .StaffEntries[verticalIndex];
      }
    }
    return undefined;
  ...
  private getFirstRightNotNullStaffEntryFromContainer(
    ...
    for (
      let i: number = horizontalIndex + 1;
      i < this.graphicalMusicSheet.VerticalGraphicalStaffEntryContainers.length;
      i++
    ) {
      if (
  ...
  private calculateWordRepetitionInstructions(): void {
    for (
      ...
      for (
        let idx: number = 0,
          len: number = sourceMeasure.FirstRepetitionInstructions.length;
        idx < len;
        ++idx
      ) {
        const instruction: RepetitionInstruction =
          sourceMeasure.FirstRepetitionInstructions[idx];
        this.calculateWordRepetitionInstruction(instruction, i);
      }
      for (
        let idx: number = 0,
          len: number = sourceMeasure.LastRepetitionInstructions.length;
        idx < len;
        ++idx
      ) {
        const instruction: RepetitionInstruction =
          sourceMeasure.LastRepetitionInstructions[idx];
        this.calculateWordRepetitionInstruction(instruction, i);
      }
    }
  }
  ...
  private calculateRepetitionEndings(): void {
    const musicsheet: MusicSheet = this.graphicalMusicSheet.ParentMusicSheet;
    for (
      let idx: number = 0, len: number = musicsheet.Repetitions.length;
      idx < len;
      ++idx
    ) {
  ...
  private calculateTempoExpressions(): void {
    ...
    for (let i: number = 0; i < maxIndex; i++) {
      ...
      for (let j: number = 0; j < sourceMeasure.TempoExpressions.length; j++) {
        this.calculateTempoExpressionsForMultiTempoExpression(
          sourceMeasure,
          sourceMeasure.TempoExpressions[j],
          i
        );
      }
    }
      ...
      for (
        ...
        if (
          ...
          for (
            ...
            if (
              sourceMeasure.StaffLinkedExpressions[j][k].MoodList.length > 0 ||
              sourceMeasure.StaffLinkedExpressions[j][k].UnknownList.length > 0
            ) {
              this.calculateMoodAndUnknownExpression(
                sourceMeasure.StaffLinkedExpressions[j][k],
                i,
                j
              );
  ...
  private calculateStemDirectionFromVoices(voiceEntry: VoiceEntry): void {
    ...
    const hasLink: boolean =
      voiceEntry.ParentSourceStaffEntry.Link !== undefined;
    if (hasLink) {
      ...
      if (beam !== undefined) {
        ...
        for (const note of beam.Notes) {
          if (note.ParentVoiceEntry === voiceEntry) {
            ...
            note.ParentVoiceEntry.WantedStemDirection !==
            StemDirectionType.Undefined
          ) {
            // set the stem direction
            voiceEntry.WantedStemDirection =
              note.ParentVoiceEntry.WantedStemDirection;
            break;
          }
        }
