...
export abstract class MusicSheetDrawer {
  ...
  public drawableBoundingBoxElement: string =
    process.env.DRAW_BOUNDING_BOX_ELEMENT;
    ...
    if (this.drawingParameters.drawCursors) {
      for (const line of graphicalMusicSheet.Cursors) {
        ...
        if (this.isVisible(psi)) {
          this.drawLineAsVerticalRectangle(line, <number>(
            GraphicalLayers.Cursor
          ));
        }
      }
    }
  ...
  public drawLineAsHorizontalRectangle(
    line: GraphicalLine,
    layer: number
  ): void {
    let rectangle: RectangleF2D = new RectangleF2D(
      line.Start.x,
      line.End.y - line.Width / 2,
  ...
  public drawLineAsVerticalRectangle(line: GraphicalLine, layer: number): void {
    ...
    let rectangle: RectangleF2D = new RectangleF2D(
      lineStart.x - lineWidth / 2,
      lineStart.y,
      lineWidth,
      line.End.y - lineStart.y
    );
    rectangle = this.applyScreenTransformationForRect(rectangle);
    this.renderRectangle(rectangle, layer, line.styleId);
  }

  public drawLineAsHorizontalRectangleWithOffset(
    line: GraphicalLine,
    offset: PointF2D,
    layer: number
  ): void {
  ...
  public drawLineAsVerticalRectangleWithOffset(
    line: GraphicalLine,
    offset: PointF2D,
    layer: number
  ): void {
  ...
  public drawRectangle(rect: GraphicalRectangle, layer: number): void {
    ...
    let rectangle: RectangleF2D = new RectangleF2D(
      psi.AbsolutePosition.x,
      psi.AbsolutePosition.y,
      psi.BorderRight,
      psi.BorderBottom
    );
    rectangle = this.applyScreenTransformationForRect(rectangle);
  ...
  public drawLabel(graphicalLabel: GraphicalLabel, layer: number): void {
    ...
    const widthInPixel: number =
      heightInPixel *
      this.textMeasurer.computeTextWidthToHeightRatio(
        label.text,
        label.font,
        label.fontStyle
      );
  ...
  protected renderSystemToScreen(
    system: MusicSystem,
    systemBoundingBoxInPixels: RectangleF2D,
    absBoundingRectWithMargin: RectangleF2D
  ): void {
    // empty
  }
  ...
  protected drawMusicSystem(system: MusicSystem): void {
    const absBoundingRectWithMargin: RectangleF2D = this.getSystemAbsBoundingRect(
      system
    );
    const systemBoundingBoxInPixels: RectangleF2D = this.getSytemBoundingBoxInPixels(
      absBoundingRectWithMargin
    );
    this.drawMusicSystemComponents(
      system,
      systemBoundingBoxInPixels,
      absBoundingRectWithMargin
    );
  }
  ...
  protected getSytemBoundingBoxInPixels(
    ...
    const systemBoundingBoxInPixels: RectangleF2D = this.applyScreenTransformationForRect(
      absBoundingRectWithMargin
    );
  ...
  protected getSystemAbsBoundingRect(system: MusicSystem): RectangleF2D {
    const relBoundingRect: RectangleF2D =
      system.PositionAndShape.BoundingRectangle;
    const absBoundingRectWithMargin: RectangleF2D = new RectangleF2D(
      system.PositionAndShape.AbsolutePosition.x +
        system.PositionAndShape.BorderLeft -
        1,
      system.PositionAndShape.AbsolutePosition.y +
        system.PositionAndShape.BorderTop -
        1,
      relBoundingRect.width + 6,
      relBoundingRect.height + 2
    );
  ...
  protected drawMusicSystemComponents(
    ...
    const selectStartSymb: SelectionStartSymbol = this.graphicalMusicSheet
      .SelectionStartSymbol;
    const selectEndSymb: SelectionEndSymbol = this.graphicalMusicSheet
      .SelectionEndSymbol;
    if (this.drawingParameters.drawSelectionStartSymbol) {
      if (
        selectStartSymb !== undefined &&
        this.isVisible(selectStartSymb.PositionAndShape)
      ) {
    ...
    if (this.drawingParameters.drawSelectionEndSymbol) {
      if (
        selectEndSymb !== undefined &&
        this.isVisible(selectEndSymb.PositionAndShape)
      ) {
  ...
  protected drawLyricLines(
    ...
    staffLine.LyricLines.forEach(lyricLine => {
      ...
      this.drawGraphicalLine(
        lyricLine,
        EngravingRules.Rules.LyricUnderscoreLineWidth
      );
    });
  }
  ...
  protected drawGraphicalLine(
    graphicalLine: GraphicalLine,
    lineWidth: number,
    colorOrStyle: string = "black"
  ): void {
  ...
  protected drawDashes(lyricsDashes: GraphicalLabel[]): void {
    lyricsDashes.forEach(dash =>
      this.drawLabel(dash, <number>GraphicalLayers.Notes)
    );
  }
  ...
  protected drawStaffLines(staffLine: StaffLine): void {
    if (staffLine.StaffLines !== undefined) {
      ...
      for (let i: number = 0; i < 5; i++) {
        this.drawLineAsHorizontalRectangleWithOffset(
          staffLine.StaffLines[i],
          position,
          <number>GraphicalLayers.Notes
        );
      }
    }
  }
  ...
  protected drawInstantaneousDynamic(
    instantaneousDynamic: GraphicalInstantaneousDynamicExpression
  ): void {
    throw new Error("not implemented");
  }
  ...
  protected drawSymbol(
    symbol: MusicSymbol,
    symbolStyle: MusicSymbolDrawingStyle,
    position: PointF2D,
    scalingFactor: number = 1,
    layer: number = <number>GraphicalLayers.Notes
  ): void {
    //empty
  }
  ...
  private drawPage(page: GraphicalMusicPage): void {
    ...
    if (this.drawableBoundingBoxElement) {
      this.drawBoundingBoxes(
        page.PositionAndShape,
        0,
        this.drawableBoundingBoxElement
      );
    }
  }
  ...
  private drawBoundingBoxes(
    ...
    const dataObjectString: string = (startBox.DataObject.constructor as any)
      .name;
    if (dataObjectString === type || type === "all") {
      let tmpRect: RectangleF2D = new RectangleF2D(
        startBox.AbsolutePosition.x + startBox.BorderMarginLeft,
      ...
      this.drawLineAsHorizontalRectangle(
        new GraphicalLine(
          new PointF2D(
            startBox.AbsolutePosition.x - 1,
            startBox.AbsolutePosition.y
          ),
          new PointF2D(
            startBox.AbsolutePosition.x + 1,
            startBox.AbsolutePosition.y
      ...
      this.drawLineAsVerticalRectangle(
        new GraphicalLine(
          new PointF2D(
            startBox.AbsolutePosition.x,
            startBox.AbsolutePosition.y - 1
          ),
          new PointF2D(
            startBox.AbsolutePosition.x,
            startBox.AbsolutePosition.y + 1
      ...
      this.renderRectangle(
        tmpRect,
        <number>GraphicalLayers.Background,
        layer,
        0.5
      );
      this.renderLabel(
        new GraphicalLabel(
          new Label(dataObjectString),
          0.8,
    ...
    layer++;
    startBox.ChildElements.forEach(bb =>
      this.drawBoundingBoxes(bb, layer, type)
    );
  }
    ...
    for (const markedArea of system.GraphicalMarkedAreas) {
      if (markedArea !== undefined) {
        if (markedArea.systemRectangle !== undefined) {
          this.drawRectangle(markedArea.systemRectangle, <number>(
            GraphicalLayers.Background
          ));
        }
        ...
        if (markedArea.labelRectangle !== undefined) {
          this.drawRectangle(markedArea.labelRectangle, <number>(
            GraphicalLayers.Background
          ));
        }
  ...
  private drawStaffLineSymbols(staffLine: StaffLine): void {
    ...
    if (this.drawingParameters.drawErrors) {
      for (const measure of staffLine.Measures) {
        ...
        if (
          measure.hasError &&
          this.graphicalMusicSheet.ParentMusicSheet.DrawErroneousMeasures
        ) {
          this.drawLineAsHorizontalRectangle(
            new GraphicalLine(
